...
describe('Pinksale Project=======', function () {
  ...
  beforeEach('This is Pinksale contract test..', async () => {
    ...
    // presale related:
    const presaleContract = await ethers.getContractFactory('PresaleNew');
    presale = await presaleContract.deploy(owner.address, rewardToken.address, 100, 100000, 1000000, 100000000, 1000000000, false, 100000, 110000)
    await presale.deployed()

    const presaleFactoryContract = await ethers.getContractFactory('PresaleFactory')
    presaleFactory = await presaleFactoryContract.deploy()
  ...
  describe('Pinksale Presale Contract Test', async () => {
    ...
    it('making two presale contract', async function () {
      const tokenAddress1 = await presaleFactory.createPresale(rewardToken.address, 100, 100000, 1000000, 100000000, 1000000000, false, 100000, 110000, {value: ethers.utils.parseEther('0.1')});
      const tokenAddress2 = await presaleFactory.createPresale(rewardToken.address, 100, 100000, 1000000, 100000000, 1000000000, false, 100000, 110000, {value: ethers.utils.parseEther('0.1')});

      let contracts = await presaleFactory.getPresales()

      console.log("presale addr 1: ", contracts[0]);
      console.log("presale addr 2: ", contracts[1]);
