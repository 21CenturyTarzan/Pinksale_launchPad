...
contract PresaleFactory {
  ...
  PresaleNew[] public presaleTokens;
  ...
  ) external payable enoughFee {
    refundExcessiveFee();
    PresaleNew newPresale = new PresaleNew(
      msg.sender, _sale_token, _token_rate, _raise_min, _raise_max,
      _softcap, _hardcap, _whitelist,
      _presale_start, _presale_end
    );
    presaleTokens.push(newPresale);
    payable(feeTo).transfer(flatFee);
  }

  function getPresales() public view returns(PresaleNew[] memory) {
